...
class DataBase:
    ...
    def donateClothesDetails(self,location,category,mode):
        query = "INSERT INTO clothes_donation(username,address,phonenumber,location,category,mode_of_transport) VALUES(%s,%s,%s,%s,%s,%s)"
        args = (session['username'], session['address'], session['phonenumber'],location,category,mode)
        self.cur.execute(query,args)
        self.dbcon.commit()
    ...
    def leftoverDetailsToDb(self,testList):
        query = "INSERT INTO leftover_food_donation(donar,donar_email,donar_address,donar_phonenumber,location,category,donation_mode) VALUES(%s,%s,%s,%s,%s,%s,%s)"
        args = (session['username'], session['email'], session['address'], session['phonenumber'], testList[0],testList[1],testList[2])
        self.cur.execute(query,args)
        self.dbcon.commit()
        pass
